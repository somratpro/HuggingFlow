...
KEEPALIVE_STATUS_FILE = Path("/tmp/huggingflow-cloudflare-keepalive-status.json")
...
def slugify(value: str) -> str:
    ...
    return (cleaned or "huggingflow-proxy")[:63].rstrip("-")
...
async function ping(source) {{
  ...
  try {{
    const response = await fetch(TARGET_URL, {{
      method: "GET",
      headers: {{
        "user-agent": "HuggingFlow Cloudflare KeepAlive",
